...
RSpec.describe TrnResponseComponent, type: :component do
  subject(:component) { render_inline(described_class.new(trn_response: trn_response)) }
  ...
  let(:trn_response) { TrnResponse.new(trn_request: trn_request, raw_result: raw_result) }
  ...
  context 'with anonymised data' do
    subject(:component) { render_inline(described_class.new(trn_response: trn_response, anonymise: true)) }
