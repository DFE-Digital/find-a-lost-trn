...
RSpec.describe PerformanceStats do
  before { Timecop.freeze(DateTime.new(2022, 5, 12, 12, 0, 0)) }
  ...
  describe '#live_service_usage' do
    it 'calculates live service usage' do
      ...
      count, data = described_class.new(longer_than_last_7_days).live_service_usage
  ...
  describe '#submission results' do
    it 'calculates submission stats' do
      ...
      count, data = described_class.new(longer_than_last_7_days).submission_results
  ...
  def given_there_are_a_few_trns
    (0..8).each.with_index do |n, i|
      (i + 1).times { create(:trn_request, created_at: n.days.ago.beginning_of_day, trn: n.odd? ? '1234567' : nil) }
    end
  end
end
