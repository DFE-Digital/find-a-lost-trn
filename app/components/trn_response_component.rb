...
class TrnResponseComponent < ViewComponent::Base
  ...
  def awarded_qts?
    ...
    (trn_response.raw_result['qualified_teacher_status']['state'] == 'Active') != trn_request.awarded_qts
  end
  ...
  def awarded_qts_value
    ...
    return 'Yes' if trn_response.raw_result['qualified_teacher_status']['state'] == 'Active'
  ...
  def itt_provider?
    ...
    (trn_response.raw_result['initial_teacher_training']['state'] == 'Active') != trn_request.itt_provider_enrolled
  end

  def itt_provider_value
    return nil if trn_response.raw_result['initial_teacher_training'].nil?

    trn_response.raw_result['initial_teacher_training']['state'] == 'Active' ? 'Yes' : 'No'
  end
  ...
  def name
    ...
    if anonymise
      trn_response.raw_result['name'].split.map { |name| "#{name.first}****" }.join(' ')
    else
